...
#define CheckNMatrixType(v)   if (TYPE(v) != T_DATA || ((RDATA(v)->dfree != (RUBY_DATA_FUNC)nm_delete && RDATA(v)->dfree != (RUBY_DATA_FUNC)nm_delete_ref) && (RDATA(v)->dfree != (RUBY_DATA_FUNC)nm_dense_storage_delete && RDATA(v)->dfree != (RUBY_DATA_FUNC)nm_dense_storage_delete_ref))) rb_raise(rb_eTypeError, "expected NMatrix on left-hand side of operation");
